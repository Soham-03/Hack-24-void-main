...
const UnplugLanding: React.FC = () => {
  ...
  const acceptanceLetterUrl = "/ff.pdf"
  ...
  const calculateTimeLeft = useCallback(() => {
    const targetDate = new Date('2025-02-18T11:00:00+05:30'); // Specific event date and time
    const now = new Date();
  ...
  const CountdownUnit: React.FC<CountdownUnitProps> = ({ value, prevValue, label }) => {
    ...
    return (
      <div className="group relative overflow-hidden flex flex-col items-center justify-center">
        <div className="relative w-24 h-24 flex items-center justify-center text-7xl font-bold mb-2 
        bg-gray-800 rounded-xl shadow-lg hover:scale-105 transition-all duration-300">
          <span 
            className={`block text-blue-400 ${
              hasChanged ? 'animate-slide-up' : ''
            }`}
          >
            {value.toString().padStart(2, '0')}
          </span>
        </div>
        <div className="text-lg text-gray-500 uppercase tracking-widest group-hover:text-blue-400 transition-colors duration-300">
          {label}
        </div>
      </div>
    );
  ...
  UNPLUG
        ...
        >
          <p className="text-3xl text-gray-400 tracking-wide transition-all duration-500">
      ...
      className="relative space-y-12 transform-gpu"
    >
      <p className="text-4xl text-center tracking-wide bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-400">
        confirm your spot in
      </p>
      
      <div className="grid grid-cols-4 gap-8 relative">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 blur-3xl opacity-50 -z-10" />
